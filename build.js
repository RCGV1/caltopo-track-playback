...
<body>
  ...
  <script>
    ...
    async function loadFromInput() {
      ...
      try {
        const response = await fetch(API_BASE + "/api/playback?url=" + encodeURIComponent(input) + "&_=" + Date.now(), { cache: "no-store" });
    ...
    function startPlayback(data) {
      ...
      for (const track of data.tracks) {
        const layer = { line: L.polyline([], { color: track.color, weight: 3, opacity: .78 }).addTo(map), marker: L.marker([track.points[0].lat, track.points[0].lng], { icon: markerIcon(track, 1, 0), keyboard: false }).addTo(map), row: null };
      ...
      function fitWindowBounds() {
        ...
        for (const track of data.tracks) {
          for (const point of track.points) {
            if (point.time >= viewStart && point.time <= viewEnd) bounds.push([point.lat, point.lng]);
          }
        }
        if (bounds.length) map.fitBounds(L.latLngBounds(bounds).pad(.08));
      }
      ...
      function update(at) {
        ...
        for (const track of data.tracks) {
          ...
          const layer = layers.get(track.id), point = visible.has(track.id) && activeInWindow ? interp(track, current) : null, staleMs = Number(stale.value), age = point ? current - point.time : Infinity, fadeStart = staleMs * .7, opacity = staleMs > 0 && age > fadeStart ? Math.max(0, 1 - ((age - fadeStart) / Math.max(1, staleMs - fadeStart))) : 1;
      ...
      for (const track of data.tracks) {
        ...
        for (const point of track.points) {
          ...
          if (!item) {
            ...
            item = { key, start: startDate, end: startDate + 86400000 - 1, points: 0, tracks: 0, seen: new Set() };
            dayMap.set(key, item);
          }
          item.points += 1;
